...
class TestFastTextStemmerIntegration(StemmerIntegrationTest):
    def test_load_trainer_from_state_sanity(self):
        trainer = FastTextStemmingTrainer(corpus_folder=self.test_corpus_folder, max_iterations=2, min_change_count=10)
        trainer.train()

        loaded_trainer = FastTextStemmingTrainer.load_from_state_file(self.test_corpus_folder)
    ...
    def test_stemmed_words_do_not_appear_in_more_then_one_iteration(self):
        trainer = FastTextStemmingTrainer(corpus_folder=self.test_corpus_folder, max_iterations=5, min_change_count=10)
        trainer.train()
    ...
    def test_get_stemmed_keyed_vectors_when_stem_dict_is_not_saved(self):
        trainer = FastTextStemmingTrainer(corpus_folder=self.test_corpus_folder, max_iterations=1)
    ...
    def test_last_completed_iteration_folder(self):
        trainer = FastTextStemmingTrainer(corpus_folder=self.test_corpus_folder, min_change_count=10)
